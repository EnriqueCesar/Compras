...
function consolidateByDateStore(rows){
  const groups=new Map();
  for(const r of rows){
    const key=`${r.dateKey}|${r.ceco||normalize(r.store)}`;
    let group=groups.get(key);
    if(!group){group={date:r.date,dateKey:r.dateKey,store:r.store,ceco:r.ceco,quantity:0,total:0,ingredients:new Set(),units:new Set()};groups.set(key,group);}
    group.quantity+=r.quantity;group.total+=r.total;if(r.ingredient)group.ingredients.add(r.ingredient);if(r.unit)group.units.add(r.unit);
  }
  return [...groups.values()].map(group=>{
    const ingredients=[...group.ingredients].sort((a,b)=>a.localeCompare(b,'es',{numeric:true}));
    const units=[...group.units].sort((a,b)=>a.localeCompare(b,'es',{numeric:true}));
    return {...group,ingredient:ingredients.length===1?ingredients[0]:`${ingredients.length} ingredientes`,ingredientTitle:ingredients.join(' · '),unit:units.length===1?units[0]:`${units.length} unidades`,unitTitle:units.join(' · ')};
  });
}
function sortRows(rows){const {key,dir}=state.sort;return rows.slice().sort((a,b)=>{let x=a[key],y=b[key];if(x instanceof Date)x=x.getTime();if(y instanceof Date)y=y.getTime();if(typeof x==='string')return x.localeCompare(y,'es',{numeric:true})*dir;return((x||0)-(y||0))*dir;});}
function sortedRows(){return sortRows(state.filtered);}
function renderTable(){const rows=sortRows(consolidateByDateStore(state.filtered)),pages=Math.max(1,Math.ceil(rows.length/PAGE_SIZE));state.page=Math.min(state.page,pages);const start=(state.page-1)*PAGE_SIZE,visible=rows.slice(start,start+PAGE_SIZE),totals=rows.reduce((acc,r)=>({units:acc.units+r.quantity,total:acc.total+r.total}),{units:0,total:0});$('tableBadge').textContent=`${fmtNum.format(rows.length)} compras consolidadas`;$('subtotalQuantity').textContent=fmtNum.format(totals.units);$('subtotalPurchase').textContent=fmtMoney.format(totals.total);$('detailSubtotalQuantity').textContent=fmtNum.format(totals.units);$('detailSubtotalPurchase').textContent=fmtMoney.format(totals.total);$('purchaseRows').innerHTML=visible.map(r=>`<tr><td><strong>${escapeHtml(r.store)}</strong></td><td title="${escapeHtml(r.ingredientTitle)}">${escapeHtml(r.ingredient)}</td><td title="${escapeHtml(r.unitTitle)}">${escapeHtml(r.unit||'Sin información')}</td><td class="num">${fmtNum.format(r.quantity)}</td><td class="num"><strong>${fmtMoney.format(r.total)}</strong></td><td>${fmtDate.format(r.date)}</td></tr>`).join('')||'<tr><td colspan="6" class="empty-state">No hay compras con los filtros actuales.</td></tr>';$('pageState').textContent=`Página ${state.page} de ${pages}`;$('prevPage').disabled=state.page<=1;$('nextPage').disabled=state.page>=pages;}
